...
class Shop{
    function  run($params){                      
        ...
        if(preg_match('/([A-Za-z0-9]+)\-([0-9]+).html$/',$params['request_uri'],$matches)){ 	            
            ...
            $params['subMenuList']      = Webshop::getLeftMenuById($params['current_webshop_id'], $parent, $params['lang']);
            ...
            if(!empty($params['subMenuList']['data']['models'])){
                ...
                $params['crumble_group'][1] = array(
                    'lbl' => $params['subMenuList']['data']['brand'],
                    'lnk' => $linkL1
                );
                $params['crumble_group'][2] = array(
                    'lbl' => $smLabel,
                    'lnk' => $params['request_uri']
                );
            }
            if($rootParent)
            {
            ...
            $params['subSubMenuList']      = Webshop::getTopSubMenuById($params['current_webshop_id'] ,$subsubMenuId, $params['lang'],$params['subMenuId']);
            if(isset($params['subSubMenuList']['data']['parent_item']) && !empty($params['subSubMenuList']['data']['parent_item']))
            {
                $params['title_override'] = $params['subSubMenuList']['data']['parent_item'].' '.strtolower($params['subSubMenuList']['data']['brand'].' - x'.$params['subSubMenuList']['data']['model']);
                $linkL1 = '/'.$params['lang'].'/'.$params['subSubMenuList']['data']['parent_item_enc'].'.html';
                $linkL2 = '/'.$params['lang'].'/shop/'.$params['subSubMenuList']['data']['parent_item_enc'].'-'.$params['subSubMenuList']['data']['brand_lower_enc'].'-'.$subsubMenuId.'.html';
        ...
        $params['cms_page'] = null;
        if($subMenuId)
        {
            $aFilters['wm.id'] = $subMenuId;
            $aCmsPages = Webshopcms::getBy('fk_menu_tree', $aFilters['wm.id'], $params['lang'], 1);
            if(isset($aCmsPages[0]))
            {
                $params['cms_page'] = $aCmsPages[0];
            }
        }
        ...
        $aFilters['has_exact_stock']      = true;
        $aFilters['in_webshop']           = 1;
        $params['products']             = ProductDao::find($aFilters,'c.id DESC',null,$params['current_page'],12,false, $groupBy);
        ...
        if(is_array($params['cms_page']))
        {
            $params['main'] = parse('inc/products-cms-page',$params);
        }
        else
        {
